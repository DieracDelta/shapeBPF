...
fn build_ebpf(target: &str, _release: bool) -> Result<()> {
    ...
    // Phase 2: Build C eBPF (qdisc struct_ops) via clang
    build_c_qdisc(&workspace_root)?;

    // Phase 3: Build C eBPF (cgroup_skb ingress observer) via clang
    build_c_ingress(&workspace_root)?;

    Ok(())
}

fn build_c_qdisc(workspace_root: &PathBuf) -> Result<()> {
    let ebpf_dir = workspace_root.join("shapebpf-ebpf");
    let src = ebpf_dir.join("src/bpf/qdisc.bpf.c");
    let out_dir = ebpf_dir.join("target/bpf");
    let out = out_dir.join("qdisc.bpf.o");

    std::fs::create_dir_all(&out_dir).context("creating target/bpf directory")?;

    // Generate vmlinux.h if it doesn't exist
    let vmlinux_h = ebpf_dir.join("src/bpf/vmlinux.h");
    if !vmlinux_h.exists() {
    ...
    let status = cmd.status().context("failed to compile C qdisc BPF program")?;
    if !status.success() {
        bail!("C qdisc BPF compilation failed with status: {}", status);
    }

    Ok(())
}

fn build_c_ingress(workspace_root: &PathBuf) -> Result<()> {
    let ebpf_dir = workspace_root.join("shapebpf-ebpf");
    let src = ebpf_dir.join("src/bpf/ingress.bpf.c");
    let out_dir = ebpf_dir.join("target/bpf");
    let out = out_dir.join("ingress.bpf.o");

    std::fs::create_dir_all(&out_dir).context("creating target/bpf directory")?;

    // vmlinux.h should already be generated by build_c_qdisc
    let target_arch = match std::env::consts::ARCH {
        "x86_64" => "__TARGET_ARCH_x86",
        "aarch64" => "__TARGET_ARCH_arm64",
        other => bail!("unsupported architecture for C eBPF: {other}"),
    ...
    let status = cmd.status().context("failed to compile C ingress BPF program")?;
    if !status.success() {
        bail!("C ingress BPF compilation failed with status: {}", status);
    }
